...
public class Particle : MonoBehaviour {
    ...
    public void FrameDelayedControlledFire(float duration) {
        DelayedControlledFire(0f, duration);
    }

    public void DelayedControlledFire(float delay, float duration) {
        GetComponent<SpriteRenderer>().enabled = false;
        ControlledFire(duration);
        isPaused = true;
        StartCoroutine(IEDelayedControlledFire(delay, duration));
    }

    IEnumerator IEDelayedControlledFire(float delay, float duration) {
